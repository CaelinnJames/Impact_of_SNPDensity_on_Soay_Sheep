...
plink --bfile ../../InputData/HD_QC --sheep --not-chr ${i},X --keep MetacarpalLength_LambPhenotypes.txt --make-bed --out HD/NoChr${i}  
  
dissect.mpich  --make-grm --bfile HD/NoChr${i} --out HD/NoChr${i}_GRM  
  
echo "HD/Chr${i} HD/NoChr${i}_GRM" > HD/GWAS_GRM${i}.txt  
  
plink --bfile ../../InputData/HD_QC --sheep --chr ${i} --keep MetacarpalLength_LambPhenotypes.txt --make-bed --out HD/Chr${i}  
...
dissect.mpich  --gwas --bfile-grm-list HD/GWAS_GRM${i}.txt --pheno MetacarpalLength_LambPhenotypes.txt --covar MetacarpalLength_LambAssoc.txt --qcovar MetacarpalLength_LambQAssoc.txt --random-effects MetacarpalLength_LambRandom.txt --random-effects-cols 1 2  --out HD/MetacarpalLength_Lamb_HD_GWAS_Chr${i}  
