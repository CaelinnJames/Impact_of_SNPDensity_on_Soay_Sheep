...
plink --bfile ../InputData/Plates_1to87_QC3 --sheep --extract SNPs_50K.txt --keep JawLength_AdultPhenotypes.txt --recodeA --out AdultHLSNP 
cut -d ' ' -f 3-6 --complement AdultHLSNP.raw > AdultHLSNP.txt
sed '1d' AdultHLSNP.txt > AdultHLSNP2.txt 

sort -k 2,2 JawLength_AdultQAssoc.txt > Assoc1.txt  
sort -k 2,2 AdultHLSNP2.txt > Assoc2.txt  

R <  ConditionalAnalysisPrep.R --save

sort -k 2,2 JawLength_AdultAssoc.txt > Assoc_50K.txt  
...
plink --bfile ../InputData/HD_QC --sheep --extract SNPs_HD.txt --keep JawLength_AdultPhenotypes.txt --recodeA --out AdultHLSNP
cut -d ' ' -f 3-6 --complement AdultHLSNP.raw > AdultHLSNP.txt
sed '1d' AdultHLSNP.txt > AdultHLSNP2.txt
...
sort -k 2,2 JawLength_AdultAssoc.txt > Assoc_HD.txt
