files = intersect(list.files(pattern="LambForeLeg_HD_GWAS_Chr",recursive=TRUE),list.files(pattern=".gwas.snps",recursive=TRUE))
...
files = intersect(list.files(pattern="LambForeLeg_50K_GWAS_Chr",recursive=TRUE),list.files(pattern=".gwas.snps",recursive=TRUE))
SNPresults = do.call(rbind, lapply(files, function(x) read.table(x,header=T)))
